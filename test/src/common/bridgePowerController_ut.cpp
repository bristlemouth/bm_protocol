...
TEST_F(BridgePowerControllerTest, goldenPath) {
    ...
    EXPECT_EQ(fake_io_write_func_fake.call_count, 1); // Init sequence powers the bus on for two minutes
    ...
    BridgePowerController._update();
    EXPECT_EQ(fake_io_write_func_fake.call_count, 2); // Init sequence powers the bus on for two minutes
    EXPECT_EQ(fake_io_write_func_fake.arg1_history[0], 1);
