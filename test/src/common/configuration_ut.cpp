...
// Second set. (overwrite)
  ...
  // int32_t
    ...
    EXPECT_EQ(config.setConfig("bar", bar),true);
...
//  float
    float baz = 3.14159;
    float result_baz = 0;
    EXPECT_EQ(config.setConfig("baz", baz),true);
    EXPECT_EQ(config.getConfig("baz", result_baz),true);
...
// str
    ...
    EXPECT_EQ(config.getConfig("silly", result_silly, size),true);
...
// Bytes
    ...
    EXPECT_EQ(config.setConfig("bytes", bytes, sizeof(bytes)),true);
    EXPECT_EQ(config.getConfig("bytes", result_bytes, size),true);
...
// Remove key
    EXPECT_EQ(config.removeKey("foo"), true);
    EXPECT_EQ(config.getConfig("foo", result_foo),false);
    key_list = config.getStoredKeys(num_keys);
    EXPECT_EQ(num_keys,4);
...
TEST_F(ConfigurationTest, NoKeyFound)
{
  ...
  uint32_t foo = 42;
  EXPECT_EQ(config.setConfig("foo", foo),true);
  EXPECT_EQ(config.getConfig("baz", result_foo),false);
}
...
TEST_F(ConfigurationTest, TooMuchThingy)
{
  ...
  EXPECT_EQ(config.setConfig("silly", silly),false);
  uint8_t bytes[100];
  memset(bytes, 0xa5, sizeof(bytes));
  EXPECT_EQ(config.setConfig("bytes", bytes, sizeof(bytes)),false);
}
