...
set -m
openocd -f $TOOLS_DIR/scripts/debug/stlink-mod.cfg -f $TOOLS_DIR/scripts/debug/st_nucleo_l4_rtos.cfg -f $TOOLS_DIR/scripts/debug/utils.cfg -c init -c halt > /dev/null 2>&1 &
openocd_pid=$!
set +m
...
# Only kill openOCD if it's still longer running
