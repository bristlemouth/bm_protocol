 ...
 */
void SensorWatchdogInit(void) {
    _sensor_watchdog_list_mutex = xSemaphoreCreateMutex();
}
 ...
 * @param[in] max_triggers The maximum number of times the sensor watchdog can expire before it is stopped.
 * @param[in] logHandle The spotter BM log handle to log to. If NULL, no logging will be done.
 */
void SensorWatchdogAdd(const char * id, uint32_t timeoutMs, sensor_watchdog_handler handler, uint32_t max_triggers, const char * logHandle) {   
    configASSERT(handler);
    configASSERT(timeoutMs > 0);
    configASSERT(id);
    sensor_watchdog_t * cfg = static_cast<sensor_watchdog_t *>(pvPortMalloc(sizeof(sensor_watchdog_t)));
    configASSERT(cfg);
    cfg->_id = id;
    ...
    cfg->_triggerCount = 0;
    cfg->_timerHandle =  xTimerCreate(cfg->_id, pdMS_TO_TICKS(cfg->_timeoutMs), pdTRUE, NULL, _timerCallback);
    cfg->_next = NULL;
...
void SensorWatchdogPet(const char * id) {
    configASSERT(id);
    configASSERT(xSemaphoreTake(_sensor_watchdog_list_mutex, pdMS_TO_TICKS(WATCHDOG_PET_TIMEOUT_MS)) == pdTRUE);
...
static void _timerCallback(TimerHandle_t xTimer) {
    ...
    while(curr) {
        if(curr->_timerHandle == xTimer){
            configASSERT(timer_callback_handler_send_cb(_sensorWatchdogHandler, reinterpret_cast<void *>(curr), 0) == pdTRUE);
        }
        curr = curr->_next;
    }
    xSemaphoreGive(_sensor_watchdog_list_mutex);
...
static void _sensorWatchdogHandler(void *arg) {
  ...
  if(curr->_triggerCount < curr->_max_triggers) {
        ...
        configASSERT(xTimerStop(curr->_timerHandle, 0) == pdTRUE);
        if(curr->_logHandle) {
            bm_fprintf(0, curr->_logHandle, "[%s] | tick: %" PRIu64 " SensorWatchdog max trigger: %" PRIu32 " reached!\n",
