		...
		zcbor_state_t *state, const struct bm_Table_Response *input)
{
	...
	bool tmp_result = (((zcbor_list_start_encode(state, 3) && ((((zcbor_list_start_encode(state, 2) && ((zcbor_multi_encode_minmax(0, 2, &(*input)._bm_Table_Response_neighbors__bm_Neighbor_Info_count, (zcbor_encoder_t *)encode_bm_Neighbor_Info, state, (&(*input)._bm_Table_Response_neighbors__bm_Neighbor_Info), sizeof(struct bm_Neighbor_Info))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_list_end_encode(state, 2)))
