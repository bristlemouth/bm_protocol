...
static bm_service_request_node_t *_create_node(size_t service_strlen, const char *service,
                                               bm_service_reply_cb reply_cb,
                                               uint32_t timeout_ms);
...
static void _service_request_cb(uint64_t node_id, const char *topic, uint16_t topic_len,
...
void bm_service_request_init(void) {
  ...
  _bm_service_request_context.expiry_timer_handle =
      xTimerCreate("Service request expiry timer", pdMS_TO_TICKS(EXPIRY_TIMER_PERIOD_MS),
                   pdTRUE, NULL, _service_request_timer_callback);
...
bool bm_service_request(size_t service_strlen, const char *service, size_t data_len,
                        const uint8_t *data, bm_service_reply_cb reply_cb, uint32_t timeout_s) {
  bool rval = false;
  ...
  do {
    ...
    bm_service_request_node_t *node =
...
static bool _request_list_add_request(bm_service_request_node_t *node) {
  bool rval = false;
  configASSERT(node);
  if (xSemaphoreTake(_bm_service_request_context.lock,
...
static bm_service_request_node_t *_create_node(size_t service_strlen, const char *service,
                                               ...
                                               uint32_t timeout_ms) {
  bm_service_request_node_t *node =
      static_cast<bm_service_request_node_t *>(pvPortMalloc(sizeof(bm_service_request_node_t)));
  configASSERT(node);
