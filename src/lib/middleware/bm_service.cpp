...
static bm_service_list_elem_t *_service_create_list_elem(size_t service_strlen,
                                                         const char *service,
                                                         bm_service_handler service_handler);
static bool _service_sub_unsub_to_req_topic(size_t service_strlen, const char *service,
                                            bool sub);
...
bool bm_service_register(size_t service_strlen, const char *service,
  ...
  bool rval = false;
  if (xSemaphoreTake(_bm_service_context.lock,
                     pdMS_TO_TICKS(DEFAULT_SERVICE_REQUEST_TIMEOUT_MS) == pdPASS)) {
    do {
      bm_service_list_elem_t *list_elem =
...
static bm_service_list_elem_t *_service_create_list_elem(size_t service_strlen,
                                                         ...
                                                         bm_service_handler service_handler) {
  bm_service_list_elem_t *list_elem =
      static_cast<bm_service_list_elem_t *>(pvPortMalloc(sizeof(bm_service_list_elem_t)));
  configASSERT(list_elem);
  ...
  if (xSemaphoreTake(_bm_service_context.lock,
    ...
    while (current != NULL) {
      if (strncmp(current->service, topic, current->service_strlen) == 0) {
        bm_service_request_data_header_s *request_header =
            (bm_service_request_data_header_s *)data;
        ...
        size_t reply_len = MAX_BM_SERVICE_DATA_SIZE -
                           sizeof(bm_service_reply_data_header_s); // Max size of reply data
        uint8_t *reply_data = static_cast<uint8_t *>(pvPortMalloc(MAX_BM_SERVICE_DATA_SIZE));
        bm_service_reply_data_header_s *reply_header =
