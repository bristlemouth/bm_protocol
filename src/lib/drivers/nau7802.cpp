...
bool NAU7802::begin() {
  ...
  bool rval = true;
  {
    ...
    rval &= setRegister(NAU7802_ADC, 0x30); //Turn off CLK_CHP. From 9.1 power on sequencing.
    printf("post setRegister rval: %u\n", rval);
    rval &= setBit(NAU7802_PGA_PWR_PGA_CAP_EN, NAU7802_PGA_PWR); //Enable 330pF decoupling cap on chan 2. From 9.14 application circuit note.
    printf("post setBit rval: %u\n", rval);

    //This cal is disabled because it gives a really different result every time (same every time on arduino). Risk of consequences but going ahead.
    // rval &= calibrateAFE(); //Re-cal analog front end when we change gain, sample rate, or channel
...
bool NAU7802::waitForCalibrateAFE(uint32_t timeout_ms)
{
  ...
  {
    if ((timeout_ms > 0) && (((uptimeGetMicroSeconds()/1000) - begin) > timeout_ms))
    {
      break;
    }
...
bool NAU7802::readData(uint8_t registerAddress, uint8_t *value, size_t dataSize) {
...
{
  uint8_t bytes[] = {static_cast<uint8_t>(registerAddress), static_cast<uint8_t>(value)};
