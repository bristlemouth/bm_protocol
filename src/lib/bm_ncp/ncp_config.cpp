...
bool ncp_cfg_get_cb(uint64_t node_id, bm_common_config_partition_e partition, size_t key_len,
                    const char *key) {
  bool rval = false;
...
bool ncp_cfg_set_cb(uint64_t node_id, bm_common_config_partition_e partition, size_t key_len,
...
bool ncp_cfg_key_del_request_cb(uint64_t node_id, bm_common_config_partition_e partition,
...
static uint8_t *alloc_ncp_key_buffer(uint8_t num_keys, const ConfigKey_t *keys, size_t &len) {
  ...
  bm_common_config_status_key_data_t *key_data =
      reinterpret_cast<bm_common_config_status_key_data_t *>(buf);
  for (int i = 0; i < num_keys; i++) {
...
BmErr _cfg_status_request_bcmp_cb(uint8_t *payload) {
  ...
  if (payload != NULL) {
    bm_common_config_status_response_t *msg =
        reinterpret_cast<bm_common_config_status_response_t *>(payload);
...
BmErr _cfg_key_del_bcmp_cb(uint8_t *payload) {
  ...
  if (payload != NULL) {
    bm_common_config_delete_key_response_t *msg =
        reinterpret_cast<bm_common_config_delete_key_response_t *>(payload);
