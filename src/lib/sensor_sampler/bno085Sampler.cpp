...
BmErr Bno085Sampler::init() {
  ...
  // Register our callbacks with `this` as the cookie; the driver auto-enables
  // accel/gyro/mag and forwards decoded reports + async events back to us.
  return _driver.init(eventCallback, sensorCallback, this);
}
...
void Bno085Sampler::sensorCallback(void *cookie, sh2_SensorEvent_t *event) {
  Bno085Sampler *self = static_cast<Bno085Sampler *>(cookie);
