...
#define MAX_SENSOR_SAMPLES (((uint64_t)SENSOR_AGG_PERIOD_MS / sys_cfg_sensorsPollIntervalMs) + 10) // 10 minutes @1Hz + 10 extra samples for padding => 610, ~5k RAM per sensor
...
void userPressureSample(pressureSample_t pressure_sample) {
  ...
  pressure_stats.values[pressure_stats.sample_count++] = pressure_sample.pressure;
  ...
  printf("pressure stats | count: %u/%lu, min: %f, max: %f\n", pressure_stats.sample_count, MAX_SENSOR_SAMPLES-10, pressure_stats.min, pressure_stats.max);
  xSemaphoreGive(xUserDataMutex);
  return;
}
...
void userPowerSample(powerSample_t power_sample) {
  ...
  if (power_voltage_stats.sample_count >= MAX_SENSOR_SAMPLES) {
    xSemaphoreGive(xUserDataMutex);
    printf("ERR - No more room in power stats buffer, already have %lu readings!\n", MAX_SENSOR_SAMPLES);
    return;
  }
  power_voltage_stats.values[power_voltage_stats.sample_count++] = power_sample.voltage;
  ...
  power_current_stats.values[power_current_stats.sample_count++] = power_sample.current;
  ...
  printf("power stats | count: %u/%lu, min_V: %f, max_V: %f, min_I: %f, max_I: %f\n",
         power_voltage_stats.sample_count, MAX_SENSOR_SAMPLES-10,
         power_voltage_stats.min,
         power_voltage_stats.max,
         power_current_stats.min,
         power_current_stats.max);
  xSemaphoreGive(xUserDataMutex);
  return;
}
...
void userHumTempSample(humTempSample_t hum_temp_sample) {
  ...
  printf("hum-temp stats | count: %u/%lu, min_T: %f, max_T: %f, min_H: %f, max_H: %f\n",
         hum_stats.sample_count, MAX_SENSOR_SAMPLES-10,
         temp_stats.min,
         temp_stats.max,
         hum_stats.min,
         hum_stats.max);
...
void setup() {
  ...
  power_voltage_stats.values = static_cast<double *>(pvPortMalloc(sizeof(double) * MAX_SENSOR_SAMPLES));
  power_current_stats.values = static_cast<double *>(pvPortMalloc(sizeof(double) * MAX_SENSOR_SAMPLES));
  temp_stats.values = static_cast<double *>(pvPortMalloc(sizeof(double) * MAX_SENSOR_SAMPLES));
  hum_stats.values = static_cast<double *>(pvPortMalloc(sizeof(double) * MAX_SENSOR_SAMPLES));
  pressure_stats.values = static_cast<double *>(pvPortMalloc(sizeof(double) * MAX_SENSOR_SAMPLES));
  stats_print_buffer = static_cast<char *>(pvPortMalloc(sizeof(char) * 750)); // Trial and error, should be plenty of space
  rtcGet(&statsStartRtc);
  userConfigurationPartition->getConfig("sensorAggPeriodMin", strlen("sensorAggPeriodMin"), sensor_agg_period_min);
}
...
void loop(void) {
  ...
  if ((uint32_t)uptimeGetMs() - sensorStatsTimer >= SENSOR_AGG_PERIOD_MS) {
    ...
    sprintf(stats_print_buffer,
            ...
            "temp_n: %u, temp_min: %.4f, temp_max: %.4f, temp_mean: %.4f, temp_std: %.4f, "
            "hum_n: %u, hum_min: %.4f, hum_max: %.4f, hum_mean: %.4f, hum_std: %.4f, "
            "voltage_n: %u, voltage_min: %.4f, voltage_max: %.4f, voltage_mean: %.4f, voltage_std: %.4f, "
            "current_n: %u, current_min: %.4f, current_max: %.4f, current_mean: %.4f, current_std: %.4f, "
            "pressure_n: %u, pressure_min: %.4f, pressure_max: %.4f, pressure_mean: %.4f, pressure_std: %.4f",
            rtcTimeBuffer, statsStartTick, statsEndTick,
            temp_tx_data.sample_count, temp_tx_data.min, temp_tx_data.max, temp_tx_data.mean, temp_tx_data.stdev,
            hum_tx_data.sample_count, hum_tx_data.min, hum_tx_data.max, hum_tx_data.mean, hum_tx_data.stdev,
            voltage_tx_data.sample_count, voltage_tx_data.min, voltage_tx_data.max, voltage_tx_data.mean, voltage_tx_data.stdev,
            current_tx_data.sample_count, current_tx_data.min, current_tx_data.max, current_tx_data.mean, current_tx_data.stdev,
            pressure_tx_data.sample_count, pressure_tx_data.min, pressure_tx_data.max, pressure_tx_data.mean, pressure_tx_data.stdev);
    ...
    sensorStatData_t* tx_data_structs[] = {
        &temp_tx_data, &hum_tx_data, &voltage_tx_data, &current_tx_data, &pressure_tx_data};
    ...
    for (uint8_t i = 0; i < N_STAT_SENSORS; i++) {
      memcpy(tx_data + N_STAT_ELEM_BYTES*i, reinterpret_cast<uint8_t *>(tx_data_structs[i]), N_STAT_ELEM_BYTES);
    }
    //
