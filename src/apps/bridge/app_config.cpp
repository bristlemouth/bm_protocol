...
power_config_s getPowerConfigs(void) {
  ...
  if (!get_config_uint(BM_CFG_PARTITION_SYSTEM, AppConfig::SAMPLE_INTERVAL_MS,
    ...
    set_config_uint(BM_CFG_PARTITION_SYSTEM, (AppConfig::SAMPLE_INTERVAL_MS, strlen(AppConfig::SAMPLE_INTERVAL_MS),
                     pwrcfg.sampleIntervalMs);
    save_config = true;
  }
  ...
  if (!get_config_uint(BM_CFG_PARTITION_SYSTEM, AppConfig::SAMPLE_DURATION_MS,
    ...
    set_config_uint(BM_CFG_PARTITION_SYSTEM, (AppConfig::SAMPLE_DURATION_MS, strlen(AppConfig::SAMPLE_DURATION_MS),
                     pwrcfg.sampleDurationMs);
    save_config = true;
  }
  ...
  if (!get_config_uint(BM_CFG_PARTITION_SYSTEM, AppConfig::SUBSAMPLE_INTERVAL_MS,
    ...
    set_config_uint(BM_CFG_PARTITION_SYSTEM, (AppConfig::SUBSAMPLE_INTERVAL_MS, strlen(AppConfig::SUBSAMPLE_INTERVAL_MS),
                     pwrcfg.subsampleIntervalMs);
    save_config = true;
  }
  ...
  if (!get_config_uint(BM_CFG_PARTITION_SYSTEM, AppConfig::SUBSAMPLE_DURATION_MS,
    ...
    set_config_uint(BM_CFG_PARTITION_SYSTEM, (AppConfig::SUBSAMPLE_DURATION_MS, strlen(AppConfig::SUBSAMPLE_DURATION_MS),
                     pwrcfg.subsampleDurationMs);
    save_config = true;
  }
  ...
  if (!get_config_uint(BM_CFG_PARTITION_SYSTEM, AppConfig::SUBSAMPLE_ENABLED,
    ...
    set_config_uint(BM_CFG_PARTITION_SYSTEM, (AppConfig::SUBSAMPLE_ENABLED, strlen(AppConfig::SUBSAMPLE_ENABLED),
                     pwrcfg.subsampleEnabled);
    save_config = true;
  }
...
raw_pressure_config_s getRawPressureConfigs(cfg::Configuration &syscfg) {
  ...
  if (!get_config_uint(BM_CFG_PARTITION_SYSTEM, AppConfig::RBR_MAX_RAW_REPORTS,
    ...
    bridgeLogPrint(BRIDGE_CFG, BM_COMMON_LOG_LEVEL_INFO, USE_HEADER,
                   ...
                   "to config: %" PRIu32 "ms\n",
                   cfg.maxRawReports);
    set_config_uint(BM_CFG_PARTITION_SYSTEM, (AppConfig::RBR_MAX_RAW_REPORTS, strlen(AppConfig::RBR_MAX_RAW_REPORTS),
                     cfg.maxRawReports);
    save_config = true;
  }
